...
public class Mairie {
    ...
    public Vector<Location> liste_location = new Vector<Location>();
    ...
    // Méthode pour ajouter un citoyen à la liste des citoyens
    ...
    public void ajout_location(Location l) {
        l.logement.decrementerPlacesLibres();
        liste_location.add(l);
    }
    ...
    public void afficherListeLogements() {
        for (Logement logement : liste_logement) {
            System.out.println("ID du logement : " + logement.Id_logement);
            System.out.println("Superficie : " + logement.superficie);
            System.out.println("Meublé : " + logement.meuble);
            if (logement.PlacesLibres > 0) {
                System.out.println("Il reste " + logement.PlacesLibres + " places libres.");
            } else {
    ...
    public void afficherListeLocations() {
        for (Location location : liste_location) {
            System.out.println("ID du locataire : " + location.citoyen.num_id + "     " +
                               "Nom du locataire : " + location.citoyen.nom + "     " +
                               "Prénom du locataire : " + location.citoyen.prenom + "     " +
                               "ID du logement : " + location.logement.Id_logement + "     " +
...
for (Mariage m: liste_mariage){
    ...
    public Logement getLogement(int id) {
        ...
        return null; // retourne null si aucun logement avec cet ID n'est trouvé
    }

    public boolean isCitoyenInLocation(int citoyenId) {
        for (Location location : liste_location) {
            if (location.citoyen.num_id == citoyenId) {
                return true;
            }
        }
        return false; // retourne false si aucun citoyen avec cet ID n'est trouvé dans les locations
    }
    public boolean logementExiste(int id) {
        for (Logement logement : liste_logement) {
            if (logement.Id_logement == id) {
                return true;
            }
        }
        return false; // retourne false si aucun logement avec cet ID n'est trouvé
    }
