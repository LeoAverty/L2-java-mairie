...
public class Mairie {
    ...
    public boolean existMariage(Mariage m) {
        for(Mariage ma:liste_mariage) {
            if (ma.date_mariage == m.date_mariage && ma.IDf == m.IDf && ma.IDh == m.IDh)
            return true;
        }
        return false;
    }
    ...
    public boolean existeDIVORCE(Mariage m){
        for(Divorce d:liste_Divorce) {
            if(d.date_mariage==m.date_mariage&&d.idF==m.IDf&&d.idH==m.IDh)
            return true;
        }
        return false;
    }
    ...
    public String etatPersonne(int id) {
        ...
        {
            if(m.IDf==id||m.IDh==id && !existeDIVORCE(m))
                return "marier";
            if(existeDIVORCE(m)==true)
                return "Divorcee";
        }
        return "celiba";
    }
